...
class ModelAccountWishlist extends Model {
	...
	public function addWishlistBySessionId($product_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "bfr_wishlist WHERE session_id = '" . $this->session->getId() . "' AND product_id = '" . (int)$product_id . "'");

		$this->db->query("INSERT INTO " . DB_PREFIX . "bfr_wishlist SET session_id = '" . $this->session->getId() . "', product_id = '" . (int)$product_id . "', date_added = NOW()");
	}
	...
	public function deleteWishlistBySession($product_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "bfr_wishlist WHERE session_id = '" . $this->session->getId() . "' AND product_id = '" . (int)$product_id . "'");
	}
	...
	public function getWishlistBySession() {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "bfr_wishlist WHERE session_id = '" . $this->session->getId() . "'");
