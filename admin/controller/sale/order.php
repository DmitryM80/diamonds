	...
	protected function getList() {
		...
		foreach ($bfr_orders_query->rows as $order_data) {
			...
			foreach ($bfr_order_products as $product_id => $quantity) {
				...
				if ($quantity > 1) {
					$bfr_product_order_cost *= $quantity;
				}
				$order_total_cost += $bfr_product_order_cost;

				$product['product_name'] = $product_query->row['name'];
				$product['product_model'] = $product_query->row['model'];
				$product['product_quantity'] = $quantity;
				$product['product_price'] = (int)$product_query->row['price'];
				$product['product_image'] = $this->model_tool_image->resize($product_query->row['image'], 50, 50);
